...
quant_path = '/raid/lgh/aids24/EX1/ex1_llama2_7b_awq_w2_fake_test'
quant_config = { "zero_point": True, "q_group_size": 64, "w_bit": 2, "version": "fake" }
...
if quant_config["version"] == "fake" or "scale":
    ...
    for n, m in dummy_model.named_modules():
        if isinstance(m, nn.Linear) or isinstance(m, LlamaRMSNorm) or isinstance(m, nn.modules.sparse.Embedding):
            print(n)
            m.weight.data = sd["model."+n+".weight"].type(m.weight.dtype)
        else:
